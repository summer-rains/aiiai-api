...
const TopUp = () => {
  ...
  const checkOrder = async (startTime, intervalId, orderId) => {
    if (new Date().getTime() > startTime + 5 * 60 * 1000) {
      showError("查询订单状态查询超时，如果确认已支付，请联系管理员！");
      ...
      const res = await API.get('/api/user/check_order?orderId=' + orderId + "&queryFromWx=" + queryFromWx);
      if (res == undefined) {
        showError("查询订单状态失败");
        ...
        showError("查询订单状态出错了：" + res.data.message);
      }
    }
  };
        ...
        <Layout.Content>
          ...
          >
            <div style={{maxWidth: '100%', textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
              {status == "active" && (
                <Countdown
                  title='订单创建成功，请扫码支付。支付后，不要手动关闭弹窗！'
                  value={deadline}
                  onFinish={onFinish}
                />
              )}
