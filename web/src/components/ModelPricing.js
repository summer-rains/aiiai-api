...
import {Tabs as AnTabs} from "antd";
...
const ModelPricing = () => {
  ...
  const setModelsFormat = (models, groupRatio) => {
    ...
    setModels(models);
    setRenderModels(models.filter((m) => m.type === activeTab))
  };
  ...
  const onTabChange = (activeKey) => {
    ...
    setRenderModels(models.filter((m) => m.type === activeKey))
  }
    ...
    <>
      <Layout>
        ...
        <Banner 
            type="info"
            fullMode={false}
            description={<div>按量计费费用 = 分组倍率 × 模型倍率 × （提示token数 + 补全token数 × 补全倍率）/ 500000 （单位：美元）<span style={{marginLeft: 10}}><a style={{textDecoration: "underline"}} href='https://gpt-best.apifox.cn' target='_blank' rel='noreferrer'>接口文档</a></span></div>}
            closeIcon="null"
        />
        <br/>
        <Tabs type={"card"} onChange={onTabChange} collapsible>
          {CHANNEL_OPTIONS.filter((m) => m.display).map((item, index) => (
            <TabPane itemKey={item.key} tab={<span style={{color: "black"}}>{item.label}</span>} key={index}>
            </TabPane>
          ))}
        </Tabs>
        <Button
