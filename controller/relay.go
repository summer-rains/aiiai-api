...
func RelayTask(c *gin.Context) {
	...
	if relayMode < relayconstant.RelayModeRerank {
		taskErr = taskRelayHandler(c, relayMode)
		//} else if relayMode < relayconstant.RelayModeRunwayGenerations {
		//	taskErr = commonTaskRelcomayHandler(c, relayMode)
	} else {
		// 穿透请求，不保存异步任务
		taskErr = commonThroughTaskRelayHandler(c, relayMode)
	}
	if taskErr == nil {
		retryTimes = 0
	}
...
func commonTaskRelcomayHandler(c *gin.Context, relayMode int) *dto.TaskError {
	var err *dto.TaskError
	switch relayMode {
	// 提交任务，或者需要扣钱的任务
	case relayconstant.RelayModeLumaGenerations, relayconstant.RelayModeLumaExtend, relayconstant.RelayModeLumaFetchHdUrl:
		err = relay.RelayCommonTaskSubmit(c, relayMode)
	// 查询任务
	case relayconstant.RelayModeLumaTasks, relayconstant.RelayModeLumaSingleTask:
		err = relay.RelayCommonTaskFetch(c, relayMode)
	default:
		err = relay.RelayCommonTaskSubmit(c, relayMode)
	}
	return err
...
func commonThroughTaskRelayHandler(c *gin.Context, relayMode int) *dto.TaskError {
	var err *dto.TaskError
	err = relay.RelayCommonThroughTask(c, relayMode)
	return err
}
